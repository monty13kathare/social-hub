...
import { Box, House, MessageCircle, Settings, ShieldPlus, Users } from 'lucide-react';
...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  { id: 'friends', icon: <Users/>, label: 'Friends', badge: null, path: '/users' },
  { id: 'communities', icon: <Box/>, label: 'Communities', badge: null, path: '/communities' },
  ...
  { id: 'messages', icon: <MessageCircle/>, label: 'Messages', badge: "soon", path: '/messages' },
  // { id: 'bookmarks', icon: '📑', label: 'Bookmarks', badge: null, path: '/bookmarks' },
  { id: 'premium', icon: <ShieldPlus/>, label: 'Premium', badge: 'Pro', path: '/premium' },
  ...
  return (
    <div className="h-full flex flex-col bg-slate-800/95 backdrop-blur-xl border-r border-slate-700/50 ">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="p-4">
          {/* User Profile */}
          <div onClick={() => navigate("/profile")} className="flex items-center space-x-3 p-4 bg-slate-700/50 rounded-xl mb-6 hover:bg-slate-700/70 transition-all duration-200 cursor-pointer">
            ...
            <div className="space-y-2">
              {communities?.map((community, index) => (
                <div
                  ...
                  onClick={() => navigate(`/communities/${community._id}`, { state: { community } })}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm font-bold ${community.bannerColor}`}>
                    {community.name[0]}
                  </div>
                  <div className="flex-1">
                    <h4 className="text-white text-sm font-medium">{community.name}</h4>
                    <p className="text-slate-400 text-xs">{community.members?.length} members</p>
                  </div>
