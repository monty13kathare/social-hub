...
import type { CompleteProfileData, LoginData, RegisterData, VerifyOtpData } from '../../types/auth';
...
export const AuthFlow: React.FC = () => {
    ...
    const [userData, setUserData] = useState<Partial<RegisterData & VerifyOtpData & CompleteProfileData>>({});
                ...
                <div className="p-8">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-400">
                            ...
                            {currentStep === 'verify-otp' && 'Enter the OTP sent to your email'}
                            {currentStep === 'complete-profile' && 'Tell us more about yourself'}
                    ...
                    {currentStep !== 'login' && (
                        <div className="flex justify-center mb-8">
                            <div className="flex items-center space-x-4">
                                {(['register', 'verify-otp', 'complete-profile'] as AuthStep[]).map((step, index) => (
                                    <React.Fragment key={step}>
                                        <div
                                            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${currentStep === step
                                                ? 'bg-pink-600 text-white'
                                                : step === 'register' || (step === 'verify-otp' && currentStep === 'complete-profile')
                                        ...
                                        {index < 2 && (
                                            <div
                                                className={`w-12 h-1 ${(step === 'register' && currentStep === 'verify-otp') ||
                                                    (step === 'verify-otp' && currentStep === 'complete-profile')
                                                    ? 'bg-green-500'
                                                    : 'bg-gray-300'
                                                    }`}
                                            />
