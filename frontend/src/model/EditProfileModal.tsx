...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({ isOpen, onClose, user, onSave, isLoading }) => {
    const [formData, setFormData] = useState({
        name: user.name,
        username: user.username,
        bio: user.bio,
        avatar: user.avatar,
        bannerColor: user.bannerColor || 'bg-gradient-to-r from-purple-500 to-purple-700',
        ...
        if (formData.location?.country) {
            ...
            if (selectedCountry) {
                ...
                if (formData.location?.state) {
                    const selectedState = selectedCountry.states.find(s => s.name === formData.location.state);
                    if (selectedState) {
    ...
    const handleLocationChange = (field: keyof LocationData, value: string) => {
        ...
        if (field === 'country') {
            ...
            const selectedCountry = countries.find(c => c.name === formData.location.country);
            const selectedState = selectedCountry?.states.find(s => s.name === value);
            setCities(selectedState?.cities || []);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-slate-800 rounded-2xl w-full max-w-2xl border border-slate-700/50 max-h-[90vh] overflow-hidden flex flex-col">
                ...
                {/* Form */}
                <form onSubmit={handleSubmit} className="p-4 md:p-6 overflow-y-auto flex-1 custom-scrollbar">

                      {/* Avatar Upload */}
                    <div className="mb-6">
                        <label className="block text-sm font-medium text-slate-300 mb-3">Profile Picture</label>
                        <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-linear-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-base sm:text-lg font-bold overflow-hidden flex-shrink-0">
                                {avatarPreview ? (
                                        ...
                                        <div className="p-2">
                                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                                                {BANNER_COLORS.map((color) => (
                                                    ...
                                                    >
                                                        <div className="flex items-center space-x-3">
                                                            <div className={`w-8 h-8 rounded-md ${color.value} border border-slate-400/30 flex-shrink-0`}></div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <svg 
                                    className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isLocationDropdownOpen ? 'rotate-180' : ''}`}
                                    fill="none" 
                                    stroke="currentColor" 
                                    viewBox="0 0 24 24"
                                >
                    ...
                    <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 pt-4 border-t border-slate-700/50">
                        ...
                        <button
                            type="submit"
                            className={`flex-1 px-6 py-3 bg-linear-to-r ${ isLoading ? 'opacity-50 cursor-not-allowed' : ''} from-purple-600 to-pink-600 text-white rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all font-semibold text-sm md:text-base`}
                        >
