...
export default function Chat({
    ...
    const [messageStatus, setMessageStatus] = useState<'sending' | 'sent' | 'failed' | null>(null);
            ...
            if (msg.id === messageId) {
                ...
                if (existingReaction) {
                    ...
                    if (existingReaction.users.includes('current')) {
                        const updatedReactions = existingReaction.users.length > 1
                            ? msg.reactions?.map(r =>
                                r.emoji === emoji
                                    ? { ...r, users: r.users.filter(u => u !== 'current') }
                                    : r
                    ...
                    return {
                        ...msg,
                        reactions: [...(msg.reactions || []), { emoji, users: ['current'] }]
                    };
                }
            }
            return msg;
                    ...
                    return (
                        ...
                        >
                            <div className={`flex space-x-3 max-w-xs lg:max-w-md ${isOwnMessage ? 'flex-row-reverse space-x-reverse' : ''}`}>
                                ...
                                {!isOwnMessage && (
                                    <div className="w-10 h-10 bg-linear-to-r from-purple-500 to-pink-500 rounded-xl flex items-center justify-center text-white font-bold flex-shrink-0 shadow-lg">
                                        {user.avatar}
                                    </div>
                                )}
                                ...
                                <div className="group relative max-w-full">
                                    <div className={`rounded-2xl p-4 relative ${isOwnMessage
                                            ? 'bg-linear-to-r from-purple-600 to-pink-600 text-white'
                                            : 'bg-slate-800/50 text-white border border-slate-700/50'
                                        } ${isFailed ? 'opacity-70' : ''}`}>
                                        <p className="text-sm leading-relaxed break-words">{message.content}</p>
                                        <div className={`flex items-center space-x-2 mt-2 ${isOwnMessage ? 'text-purple-200' : 'text-slate-400'
                                            }`}>
                                    ...
                                    {message.reactions && message.reactions.length > 0 && (
                                        <div className={`flex space-x-1 mt-2 ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
                                            {message.reactions.map((reaction, index) => (
                                                <div
                                                    key={index}
                                                    className={`bg-slate-700/80 backdrop-blur-sm rounded-full px-2 py-1 text-xs flex items-center space-x-1 cursor-pointer hover:bg-slate-600/80 transition-colors ${reaction.users.includes('current') ? 'ring-1 ring-purple-400' : ''
                                                        }`}
                                    ...
                                    {/* Quick Reactions */}
                                    <div className={`absolute top-0 opacity-0 group-hover:opacity-100 transition-all duration-200 ${isOwnMessage ? 'left-0 -translate-x-full' : 'right-0 translate-x-full'
