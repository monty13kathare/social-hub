...
import { createCommunity, getCommunityById } from '../api/community';
...
console.log('formData', formData)
  ...
  const getSelectedColorName = () => {
    const selectedColor = BANNER_COLORS.find(color => color.value === formData.bannerColor);
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      if (!formData.name.trim()) {
        newErrors.push({ field: 'name', message: 'Community name is required' });
      } else if (formData.name.length < 3) {
        newErrors.push({ field: 'name', message: 'Community name must be at least 3 characters' });
      } else if (formData.name.length > 50) {
        newErrors.push({ field: 'name', message: 'Community name must be less than 50 characters' });
      }
    ...
    if (step === 2) {
      ...
      if (formData.rules.length === 0) {
        newErrors.push({ field: 'rules', message: 'Add at least one community rule' });
  ...
  const steps = [
    { number: 1, title: 'Basic Info', description: 'Community name and description' },
    ...
    <div className="min-h-screen bg-slate-900 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Create Community</h1>
          <p className="text-slate-400">Build your own space and connect with like-minded people</p>
        </div>
            ...
            {steps.map((step, index) => (
              <div key={step.number} className="flex items-center">
                ...
                {index < steps.length - 1 && (
                  <div
                    className={`w-16 h-1 transition-all duration-300 ${currentStep > step.number ? 'bg-purple-600' : 'bg-slate-600'
                      }`}
                  />
                )}
              </div>
        ...
        <div className="flex justify-center mb-8">
          <div className="flex space-x-8">
            {steps.map((step) => (
              <div
                key={step.number}
                className={`text-center transition-all duration-300 ${currentStep === step.number ? 'text-white' : 'text-slate-400'
                  }`}
              >
          ...
          <form onSubmit={handleSubmit}>
            ...
            {currentStep === 1 && (
              <div className="space-y-6">
                ...
                {/* Community Name */}
                <div>
                  <label htmlFor="community-name" className="block text-white font-medium mb-2">
                    Community Name *
                  </label>
                  <input
                    id="community-name"
                    ...
                    className={`w-full bg-slate-800/50 border rounded-xl py-3 px-4 text-white placeholder-slate-400 focus:outline-none focus:ring-2 transition-all duration-200 ${getError('name')
                  ...
                  />
                  {getError('name') && (
                    <p className="text-red-400 text-sm mt-2">{getError('name')}</p>
                  )}
                ...
                {/* Description */}
                <div>
                  <label htmlFor="community-description" className="block text-white font-medium mb-2">
                    Description *
                  </label>
                  <textarea
                    ...
                    rows={4}
                    className={`w-full bg-slate-800/50 border rounded-xl py-3 px-4 text-white placeholder-slate-400 focus:outline-none focus:ring-2 transition-all duration-200 resize-none ${getError('description')
                      ? 'border-red-500 focus:ring-red-500/30'
                      : 'border-slate-700/50 focus:ring-purple-500/30 focus:border-transparent'
                      }`}
                    maxLength={500}
                  ...
                  {getError('description') && (
                    <p className="text-red-400 text-sm mt-2">{getError('description')}</p>
                  )}
                  <p className="text-slate-400 text-sm mt-2">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 h-80 overflow-y-scroll custom-scrollbar p-4">
                    {mainCategories.map((category) => (
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 transition-all duration-200 text-left ${formData.category === category.id
                  ...
                  {getError('category') && (
                    <p className="text-red-400 text-sm mt-2">{getError('category')}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-white mb-2">Community Settings</h2>
                  <p className="text-slate-400">Configure rules and privacy settings</p>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                      ...
                      <button
                        key={option.value}
                        type="button"
                        onClick={() => handleInputChange('privacy', option.value as any)}
                        className={`p-4 rounded-xl border-2 transition-all duration-200 text-left ${formData.privacy === option.value
                          ? 'border-purple-500 bg-purple-600/20'
                          : 'border-slate-700/50 bg-slate-800/30 hover:border-slate-600'
                          }`}
                      >
                ...
                <div>
                  ...
                  {getError('tags') && (
                    <p className="text-red-400 text-sm mt-2">{getError('tags')}</p>
                  )}
                  <p className="text-slate-400 text-sm mt-2">
                    {formData.tags.length}/5 tags • Tags help people discover your community
                  </p>
                </div>
                ...
                <div>
                  ...
                  {getError('rules') && (
                    <p className="text-red-400 text-sm mt-2">{getError('rules')}</p>
                  )}
                  <p className="text-slate-400 text-sm mt-2">
                    {formData.rules.length}/10 rules • Clear rules help maintain a healthy community
                  </p>
                </div>
              </div>
            )}
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-white mb-2">Appearance & Permissions</h2>
                  <p className="text-slate-400">Customize how your community looks and works</p>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {bannerColors.map((color) => (
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 transition-all duration-200 ${formData.bannerColor === color.value
                          ? 'border-purple-500 ring-2 ring-purple-500/50'
                          : 'border-slate-700/50 hover:border-slate-600'
                          }`}
                      >
                        <div className={`h-12 rounded-lg ${color.preview} mb-2`} />
                        <div className="text-white text-sm text-center">{color.label}</div>
                      </button>
                    ))}
                  </div>
                </div> */}
                ...
                <div className="mb-6">
                  ...
                  <div className={`h-16 md:h-20 rounded-xl mb-4 ${formData.bannerColor}`}></div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-6 h-6 rounded-md ${formData.bannerColor} border border-slate-400/30`}></div>
                        <span className="text-sm md:text-base">{getSelectedColorName()}</span>
                      </div>
                      <svg
                        className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isColorDropdownOpen ? 'rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                              {BANNER_COLORS.map((color) => (
                                <button
                                  ...
                                  className={`p-3 rounded-lg text-left hover:bg-slate-600/50 transition-all ${formData.bannerColor === color.value
                                      ? 'bg-slate-600 ring-2 ring-purple-500'
                                      : 'bg-slate-700/50'
                                    }`}
                                >
                                  <div className="flex items-center space-x-3">
                                    <div className={`w-8 h-8 rounded-md ${color.value} border border-slate-400/30 shrink-0`}></div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      { key: 'allowImage' as const, label: 'Allow image posts', description: 'Members can share images' },
                      { key: 'allowCode' as const, label: 'Allow Code posts', description: 'Members can share Codes' },
                      { key: 'allowPoll' as const, label: 'Allow Poll posts', description: 'Members can share Polls' },
                      { key: 'allowAchievement' as const, label: 'Allow Achievement posts', description: 'Members can share Achievement' },
                      { key: 'allowLink' as const, label: 'Allow link posts', description: 'Members can share external links' },
                    ].map((permission) => (
                      <label
                        key={permission.key}
                      ...
                      >
                        ...
                        <input
                          ...
                          onChange={(e) => handleInputChange(permission.key, e.target.checked)}
                ...
                <div>
                  <label className="flex items-center justify-between p-4 rounded-xl border border-slate-700/50 bg-slate-800/30 hover:border-slate-600 transition-all duration-200 cursor-pointer">
                    <div>
                      <div className="text-white font-medium">Require post approval</div>
                      <div className="text-slate-400 text-sm">All posts must be approved by moderators before appearing</div>
                    </div>
                    <input
                      type="checkbox"
                      ...
                      <div className="flex items-center space-x-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-sm">
                            {formData.privacy === 'public' ? 'Public Community' :
                              formData.privacy === 'restricted' ? 'Restricted Community' : 'Private Community'}
                          </p>
                        </div>
                      </div>
