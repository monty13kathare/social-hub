...
import { getAllCommunities, joinCommunity, leaveCommunity } from "../api/community";
...
export default function Communities() {
    ...
    useMemo(() => {
        ...
        // Members range
        result = result.filter(community =>
            (community.membersCount || community.members?.length || 0) >= filters.minMembers &&
            (community.membersCount || community.members?.length || 0) <= filters.maxMembers
        );
        ...
        result.sort((a, b) => {
            switch (filters.sortBy) {
                ...
                case 'active':
                    return (b.onlineMembers || b.online || 0) - (a.onlineMembers || a.online || 0);
                case 'popular':
                default:
                    return (b.membersCount || b.members?.length || 0) - (a.membersCount || a.members?.length || 0);
            }
        });
    ...
    const categoryMap: Record<string, number> = communities.reduce((acc, community) => {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-slate-800/50 rounded-2xl border border-purple-500/20 backdrop-blur-xl p-6 mb-8">
                    <div className="flex flex-col lg:flex-row gap-4">
                        ...
                        {/* Clear Filters */}
                        {(filters.query || filters.category !== 'all' || filters.tags.length > 0 || filters.hasTags) && (
                            <button
                            ...
                            <div className="mt-6 flex items-center">
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input
                                        ...
                                        onChange={(e) => handleFilterChange('hasTags', e.target.checked)}
                ...
                {/* Active Filters */}
                {(filters.tags.length > 0 || filters.hasTags || filters.category !== 'all') && (
                ...
                <div className="mb-6">
                    <p className="text-slate-400">
                        Found <span className="text-white font-semibold">{filteredCommunities.length}</span> communities
                            ...
                            <div className="space-y-2">
                                {categories.map((category) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 ${filters.category === category.id
